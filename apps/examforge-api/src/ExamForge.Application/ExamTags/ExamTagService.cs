...
public sealed class ExamTagService
{
    private readonly IExamTagRepository _examTags;
    private readonly IUnitOfWork _unitOfWork;

    public ExamTagService(
        IExamTagRepository examTags,
    ...
    public async Task<Result<ExamTagResponse, ExamTagError>> GetByIdAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var tag = await _examTags.GetByIdAsync(id, includeArchived, cancellationToken);
    ...
    public async Task<Result<ExamTagResponse, ExamTagError>> GetByTypeAndSlugAsync(
        ExamTagType type,
        string slug,
        bool includeArchived,
        CancellationToken cancellationToken = default)
    {
        if (!IsValidType(type))
        {
            return Result<ExamTagResponse, ExamTagError>.Failure(ExamTagError.InvalidType);
        }

        if (string.IsNullOrWhiteSpace(slug))
        {
            return Result<ExamTagResponse, ExamTagError>.Failure(ExamTagError.InvalidSlug);
        }

        var normalizedSlug = TextNormalizer.NormalizeSlug(slug);

        var tag = await _examTags.GetByTypeAndSlugAsync(
            type,
            normalizedSlug,
            includeArchived,
            cancellationToken);
